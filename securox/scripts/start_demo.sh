...
export PYTHONPATH="${SCRIPT_DIR}/securox/backend/app:${SCRIPT_DIR}/securox:${PYTHONPATH}"

# Check dependencies
$PYTHON_CMD -c "import fastapi, uvicorn, sklearn, pydantic" 2>/dev/null || {
    echo "[*] Installing required packages..."
    pip install -r "${SCRIPT_DIR}/securox/backend/requirements.txt"
}
...
cd "${SCRIPT_DIR}/securox/backend/app"
