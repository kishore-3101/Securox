...
export const OperationalMap: React.FC<OperationalMapProps> = ({
  ...
  const DEFAULT_CENTER: [number, number] = [12.955, 77.605];
  ...
  useEffect(() => {
    ...
    activeList.forEach((sig, idx) => {
      const lat = (sig as any).latitude || (12.92 + (idx % 5) * 0.025);
      const lng = (sig as any).longitude || (77.58 + (idx % 5) * 0.02);
      const phase = ((sig as any).current_phase || (sig as any).current_state || 'GREEN').toUpperCase();
      ...
      marker.bindPopup(`
        <div style="background:#0f172a; color:#f8fafc; padding:10px; border-radius:8px; font-family:monospace; font-size:12px; border:1px solid ${color};">
          ...
          <span>Operational Mode: ${(sig as any).mode || 'ADAPTIVE'}</span><br/>
          <span>Junction ID: ${(sig as any).intersection_id || 'J-101'}</span>
        </div>
      `);
  ...
  useEffect(() => {
    ...
    activeList.forEach((cam, idx) => {
      const lat = (cam as any).latitude || (12.925 + (idx % 5) * 0.018);
      const lng = (cam as any).longitude || (77.585 + (idx % 5) * 0.015);
      ...
      marker.bindPopup(`
        <div style="background:#0f172a; color:#f8fafc; padding:10px; border-radius:8px; font-family:monospace; font-size:12px; border:1px solid ${color};">
          ...
          <span>Stream: ${(cam as any).stream_type || 'WEBRTC'} @ ${(cam as any).fps || 15} FPS</span>
        </div>
      `);
  ...
  useEffect(() => {
    ...
    activeList.forEach((amb, idx) => {
      const lat = (amb as any).latitude || (12.94 + (idx % 2) * 0.035);
      const lng = (amb as any).longitude || (77.60 + (idx % 2) * 0.025);
  ...
  useEffect(() => {
    ...
    activeList.forEach((inc, idx) => {
      const lat = (inc as any).latitude || (12.86 + (idx % 3) * 0.03);
      const lng = (inc as any).longitude || (77.65 + (idx % 3) * 0.015);
