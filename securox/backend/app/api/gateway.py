...
async def override_signal(
    ...
    """Manually override a traffic signal state with strict RBAC/ABAC verification."""
    ok = await stig.override_signal(req.junction_id, req.state)
    if not ok:
        raise HTTPException(400, f"Junction {req.junction_id} not found.")
    await store.audit(current_user["username"], "SIGNAL_MANUAL_OVERRIDE", req.junction_id, {"new_state": req.state})
...
async def override_traffic_signal(
    signal_id: str,
    req: TrafficSignalOverrideRequest,
    ...
    success = await store.update_traffic_signal(
        signal_id=signal_id, state=req.state, mode=req.mode, override_by=override_actor
    )
    if not success:
...
async def list_bank_accounts(
    ...
    current_user: dict = Depends(require_access(ResourceType.BANK_ACCOUNT, Action.VIEW))
):
    """Lists bank accounts with customer BOLA protection."""
    if current_user.get("role") == "customer":
        customer_id = current_user.get("username")
    accounts = await store.get_bank_accounts(customer_id=customer_id)
    return {"status": "ok", "total": len(accounts), "accounts": accounts}


@app.get("/api/finance/accounts/{account_id}", tags=["Finance"])
async def get_bank_account_detail(
    account_id: str,
    current_user: dict = Depends(require_access(ResourceType.BANK_ACCOUNT, Action.VIEW, object_id_param="account_id"))
):
    """Retrieves account details with strict customer BOLA/IDOR protection."""
    account = await store.get_bank_account(account_id)
    if not account:
...
async def list_bank_transactions(
    ...
    limit: int = 50,
    current_user: dict = Depends(require_access(ResourceType.TRANSACTION, Action.VIEW))
):
...
async def create_transaction_endpoint(
    req: BankTransactionCreateRequest,
    current_user: dict = Depends(require_access(ResourceType.TRANSACTION, Action.CREATE, object_id_param="account_id"))
):
    """Executes a transaction with real-time pre-settlement ML risk scoring & adaptive escrow hold."""
    sender = current_user.get("username", req.sender_name)
    # Context risk evaluation
    eval_req = AccessEvaluateRequest(
        user_id=current_user.get("id", sender),
        username=sender,
