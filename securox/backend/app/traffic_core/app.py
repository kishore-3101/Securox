...
def get_roads(db: Session = Depends(get_db)):
    ...
    for r in roads:
        res.append({
            ...
            "capacity": getattr(r, "capacity", 1500),
            "density_score": getattr(r, "density_score", 45.0),
            "congestion_level": r.congestion_level,
            "status": getattr(r, "status", "ONLINE"),
            "coordinates": json.loads(getattr(r, "coordinates_json", "[]") or "[]")
        })
    return res
...
def get_road_detail(road_id: str, db: Session = Depends(get_db)):
    ...
    cong_analysis = traffic_intelligence.calculate_congestion(
        ...
        capacity=getattr(road, "capacity", 1500),
    ...
    return {
        "road": {
            ...
            "current_volume": road.current_volume,
            "capacity": getattr(road, "capacity", 1500),
            "coordinates": json.loads(getattr(road, "coordinates_json", "[]") or "[]")
        },
...
def update_incident_status(
    ...
    db: Session = Depends(get_db),
    user: dict = Depends(require_access(ResourceType.SECURITY_INCIDENT, Action.UPDATE, object_id_param="incident_id")),
):
    try:
...
async def override_toll(
    ...
    if anomaly:
        ...
        anomaly.override_by = current_user.get("username", "operator")
