    ...
    async def dependency(
        ...
        if role == "auditor" and action not in (Action.VIEW, Action.EXPORT, Action.DOWNLOAD):
            ...
            await event_fabric.emit(
                ...
                domain=domain if domain != "GLOBAL" else "SECURITY",
                user=username,
                role=role,
                ...
                metadata={"reason": "Auditor role attempted prohibited mutating action"}
            )
            raise HTTPException(
        ...
        if not has_rbac and role not in ("admin", "superadmin"):
            ...
            await event_fabric.emit(
                ...
                risk=70.0,
                ip=request.client.host if request.client else "127.0.0.1",
                metadata={"reason": f"Role '{role}' missing permission '{action.value}' on '{resource.value}'"}
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if resource in (ResourceType.PATIENT_RECORD, ResourceType.CLINICAL_NOTE, ResourceType.PRESCRIPTION, ResourceType.LAB_REPORT):
            if role in ("doctor", "nurse") and target_object_id:
                ...
                if patient:
                    ...
                    if is_emergency:
                        # Immediately log event, increase risk, mark as BREAK_GLASS, notify security, preserve audit evidence
                        bg_reason = request.headers.get("X-Break-Glass-Reason") or "Emergency Clinical Resuscitation"
                        ...
                        await store.record_break_glass_event({
                            "user_id": current_user.get("id", username),
                            "username": username,
                            "role": role,
                            "patient_id": target_object_id,
                        ...
                        await store.add_incident({
                            ...
                            "title": f"BREAK_GLASS EMERGENCY ACCESS: {role.upper()} '{username}' -> Patient {target_object_id}",
                            "status": "OPEN",
                            "severity": "HIGH",
                    ...
                    elif role == "nurse" and not is_assigned:
                        # Nurse must only access appropriate assigned patients
                        await store.audit(
                            username,
                            "BOLA_NURSE_BLOCKED",
                            f"patient:{target_object_id}",
                            {"status": "BLOCKED", "reason": "Nurse attempted to access unassigned patient"}
                        )
                        raise HTTPException(
                            status_code=status.HTTP_403_FORBIDDEN,
                            detail=f"Access Denied (BOLA/IDOR): Nurse '{username}' may only access explicitly assigned shift patients."
                        )
                    elif not is_assigned and not dept_matches and not is_emergency:
                        await store.audit(
                            username,
        ...
        # Reception & Billing: Strictly barred from clinical notes, prescriptions, and lab diagnostic records
        if resource in (ResourceType.CLINICAL_NOTE, ResourceType.PRESCRIPTION, ResourceType.LAB_REPORT):
            if role in ("reception", "billing_staff", "billing"):
                await store.audit(
                    username,
                    "PRIVACY_CLINICAL_SHIELD_BLOCKED",
                    f"{resource.value}:{action.value}",
                    {"status": "BLOCKED", "role": role, "reason": "Non-clinical administrative role barred from clinical health records"}
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail=f"Access Denied: Role '{role}' is restricted from clinical medical records and diagnoses (HIPAA Privacy Rule)."
                )

        # Ambulance Driver: Only operational emergency information allowed
        if role == "ambulance_driver":
            if resource in (ResourceType.CLINICAL_NOTE, ResourceType.PRESCRIPTION, ResourceType.LAB_REPORT):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Access Denied: Ambulance driver may only view operational emergency dispatch data, not clinical medical history."
                )
